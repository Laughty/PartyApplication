...
class AbstractService: AbstractServiceProtocol {
    ...
    func fetchRequest<T>(with factory: Friends.FriendsPredicateFactory) -> [T]? where T: NSManagedObject {
        let moc = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
        
        let request = NSFetchRequest<T>(entityName: T.entity().name!)
        
        do {
            request.predicate = factory.predicate
            let requestResult = try moc.fetch(request)
            callLogging()
            return requestResult
        } catch {
            print("Couldn't find proper entity")
